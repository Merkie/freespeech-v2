...
registerRoute(
  ({ url }) =>
    url.hostname.endsWith('freespeechaac.com') &&
...
registerRoute(
  ({ url }) =>
    url.hostname.includes('opensymbols') ||
    url.hostname.includes('arasaac') ||
    url.hostname.includes('mulberry'),
  new CacheFirst({
    cacheName: CACHE_NAMES.images,
    plugins: [
      new CacheableResponsePlugin({
...
registerRoute(
  ({ url }) =>
    url.hostname.includes('api.freespeechaac.com') ||
