...
const EditTilePanel: Component<EditTilePanelProps> = (props) => {
  const [showingDisplayTextOption, setShowingDisplayTextOption] =
    createSignal(false);
  ...
  createEffect(() => {
    ...
    if (isBulkEditing()) {
      const hasChanges =
        pending.backgroundColor !== undefined ||
        pending.borderColor !== undefined;
    ...
    const hasChanges =
      ...
      (pending.displayText !== undefined &&
        pending.displayText !== originalTile.displayText) ||
      (pending.image !== undefined && pending.image !== originalTile.image) ||
      (pending.backgroundColor !== undefined &&
        pending.backgroundColor !== originalTile.backgroundColor) ||
      (pending.borderColor !== undefined &&
        pending.borderColor !== originalTile.borderColor) ||
      (pending.navigation !== undefined &&
        pending.navigation !== originalTile.navigation);
  ...
  const handleSaveChanges = async () => {
    ...
    if (isBulkEditing()) {
      ...
      await api.tile.update(currentPageId, { x: tile.x, y: tile.y, page: tile.page }, {
  ...
  const handleDeleteTiles = async () => {
    ...
    if (page?.tilePage) {
      ...
      const updatedTiles = currentTiles.filter(
        (t) => !positions.includes(tilePositionKey(t))
      );
      setCurrentPage({
        ...page,
        tilePage: {
  ...
  const isColorSelected = (colorKey: string) => {
    ...
    return (
      bgColor === colorValues.background_color &&
      borderColor === colorValues.border_color
    );
  };
  ...
  const isPageColorSelected = (bgColor: string, borderColor: string) => {
    return (
      getDisplayValue('backgroundColor') === bgColor &&
      getDisplayValue('borderColor') === borderColor
    );
  };
  ...
  const pageColors = () => {
    ...
    for (const tile of tiles()) {
      ...
      const isDefault = defaultColors.some(
        (c) => c.background_color === bg && c.border_color === border
      );
  ...
  const disabledInBulk = () =>
    isBulkEditing() ? 'opacity-50 pointer-events-none select-none' : '';
      ...
      <Show when={editingTilePositions().length > 0} fallback={<EmptyState />}>
          ...
          <div class="flex flex-col gap-2">
            ...
            >
              ...
              <div class="flex gap-2">
                ...
                >
                  <Show
                    when={removingBackground()}
                    fallback={<i class="bi bi-eraser-fill" />}
                  >
        ...
        <div class="flex flex-wrap gap-2 rounded-md text-black">
          <For each={Object.keys(tileColors)}>
            {(colorKey) => {
              const colorValues =
                tileColors[colorKey as keyof typeof tileColors];
              return (
                <button
        ...
        <div class={cn('mt-6', disabledInBulk())}>
          ...
          <div>
            ...
            >
              ...
              <For each={pages()}>
                {(page) => <option value={page.id}>{page.name}</option>}
              </For>
            </select>
          </div>
        </div>
        ...
        >
          <i class="bi bi-trash-fill" />
          {isBulkEditing()
            ? `Delete ${editingTilePositions().length} Tiles`
            : 'Delete Tile'}
        </button>
      </Show>
    </div>
