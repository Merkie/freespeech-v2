...
export default function TileSubpages({
  containerHeight,
}: {
  ...
  createEffect(() => {
    ...
    const maxPage =
      tiles.length > 0 ? Math.max(...tiles.map((t) => t.page)) : -1;
  ...
  const isCurrentPageATemplate = () =>
    currentPage()?.tilePage?.isTemplate === true;
  ...
  const getMergedTilesForSubpage = (
    pageTiles: TileType[],
    pageIndex: number
  ): RenderTile[] => {
            ...
            <For each={getUnusedCoords(tiles, pageIndex())}>
              {(coord) => (
                <AddTileButton
                  x={coord.x}
                  y={coord.y}
                  page={getDbPageForSubpage(tiles)}
                />
              )}
            </For>
          </Show>
