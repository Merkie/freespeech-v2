...
const InternalVoiceSelector: Component = () => {
	const [offlineBrowserVoices, setOfflineBrowserVoices] = createSignal<SpeechSynthesisVoice[]>([]);
	...
	const scrollToSelectedVoice = async () => {
		...
		if (selectedOfflineVoiceDiv && voicesContainerRef) {
			voicesContainerRef.scrollTo({
				top: selectedOfflineVoiceDiv.offsetTop - voicesContainerRef.clientHeight / 2,
				behavior: 'smooth',
			});
		}
	};
