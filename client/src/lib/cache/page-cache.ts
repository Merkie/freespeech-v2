import type { Template, Tile, TilePage } from '../types';
import { type CachedPage, getDB } from './db';
...
export async function cachePage(page: TilePage, template: Template | null, templateTiles: Tile[]): Promise<void> {
	try {
		const db = await getDB();
		const now = Date.now();
