...
import { fetchProjectWithCache, fetchPageWithCache } from './api/cached-fetch';
import {
  project,
  setCurrentPage,
  setCurrentPageId,
...
export async function loadProject(
  ...
  try {
    ...
    const { project: projectResponse, fromCache } = await fetchProjectWithCache(
      projectId,
      {
...
export async function navigateToPageInProject(
  pageId: string,
  ...
  try {
    // Use cached fetch with stale-while-revalidate strategy
    const { page, template, templateTiles, fromCache } = await fetchPageWithCache(
      pageId,
      {
        skipCache: options?.skipCache,
