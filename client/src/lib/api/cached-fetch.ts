 ...
 */
async function revalidateProject(projectId: string, onRevalidate?: (project: Project) => void): Promise<void> {
 ...
 */
async function revalidatePage(pageId: string, onRevalidate?: (result: PageWithTemplate) => void): Promise<void> {
	if (!navigator.onLine) return;
